  ...
  return (
    <div className="space-y-6 select-none">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="font-bold text-2xl md:text-3xl text-slate-100 tracking-tight">Sales & Invoices</h2>
          <p className="text-xs md:text-sm text-slate-400 mt-1">Manage your receivables and track business growth.</p>
        </div>
        <button
          onClick={onOpenNewInvoice}
          className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 hover:brightness-110 text-white rounded-xl text-xs md:text-sm font-semibold shadow-lg shadow-indigo-600/20 transition-all active:scale-[0.98]"
        >
      ...
      <div className="flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-4">
        {/* Type Selector Tabs */}
        <div className="flex items-center gap-1 glass-card bg-slate-900/80 p-1 rounded-xl border border-slate-800 shadow-level-1 w-fit">
          <button
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          <div className="relative flex-1 sm:w-64">
            ...
            <input
              ...
              className="w-full bg-slate-950/80 border border-slate-800 rounded-xl py-2 pl-10 pr-4 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 shadow-sm"
            />
          </div>
          ...
          <select
            ...
            className="bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-xs font-semibold text-slate-200 shadow-sm focus:border-indigo-500 outline-none"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {/* Total Invoiced */}
        <div className="glass-card bg-slate-900/60 rounded-2xl p-5 shadow-level-1 border border-slate-800/80 flex flex-col justify-between h-36 relative overflow-hidden group hover:border-indigo-500/50 transition-all">
          ...
          <div>
            <div className="flex items-baseline gap-1">
              ...
              <span className="font-bold text-2xl md:text-3xl text-slate-100 tracking-tight">
        ...
        {/* Amount Received */}
        <div className="glass-card bg-slate-900/60 rounded-2xl p-5 shadow-level-1 border border-slate-800/80 flex flex-col justify-between h-36 relative overflow-hidden group hover:border-emerald-500/50 transition-all">
          <div className="flex justify-between items-start">
          ...
          <div>
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-slate-400">{receivedPercent}%</span>
            </div>
          </div>
        </div>

        {/* Outstanding Balance */}
        <div className="glass-card bg-slate-900/60 rounded-2xl p-5 shadow-level-1 border border-slate-800/80 flex flex-col justify-between h-36 relative group hover:border-indigo-500/50 transition-all">
          <div className="flex justify-between items-start">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">Across {invoices.filter(i => i.status === 'pending').length} active invoices</p>
          </div>
        </div>

        {/* Overdue (Alert Card) */}
        <div className="glass-card bg-slate-900/60 rounded-2xl p-5 shadow-level-1 border border-rose-500/30 flex flex-col justify-between h-36 relative group hover:border-rose-500/60 transition-all">
          <div className="flex justify-between items-start">
      ...
      <div className="glass-card bg-slate-900/60 rounded-2xl shadow-level-1 border border-slate-800/80 overflow-hidden">
        <div className="p-5 border-b border-slate-800/80 flex justify-between items-center">
          <h3 className="font-bold text-base md:text-lg text-slate-100">Recent Invoices</h3>
              ...
              {filteredInvoices.map((inv) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right font-bold text-sm text-slate-100">
                    ...
                    <td className="px-6 py-4 text-center">
                      ...
                      {inv.status === 'pending' && (
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 uppercase">
                          Sent / Pending
                        </span>
                      )}
                      {inv.status === 'draft' && (
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-slate-800 text-slate-400 border border-slate-700 uppercase">
                          Draft
                        </span>
                      )}
                    </td>
      ...
      {selectedInvoice && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-in fade-in">
          <div className="glass-card bg-slate-900/95 rounded-2xl max-w-2xl w-full p-6 md:p-8 shadow-2xl border border-slate-800 max-h-[90vh] overflow-y-auto">
            {/* Modal Header */}
            <div className="flex justify-between items-start pb-6 border-b border-slate-800">
              <div>
                <span className="bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 text-xs font-bold px-2.5 py-1 rounded-full uppercase">
                  Tax Invoice
                </span>
                <h3 className="font-bold text-2xl text-slate-100 mt-2">{selectedInvoice.invoiceNumber}</h3>
                <p className="text-xs text-slate-400">Aiman's Cafe • SSM: 202301045678 (1523412-V)</p>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => window.print()}
                  className="p-2 rounded-xl bg-slate-800 text-slate-200 hover:bg-slate-700 transition-all border border-slate-700"
                  title="Print"
                >
            ...
            {/* Bill To & Dates */}
            <div className="grid grid-cols-2 gap-6 py-6 border-b border-slate-800 text-xs md:text-sm">
              <div>
                <span className="text-slate-400 font-semibold uppercase text-[11px] block mb-1">Billed To:</span>
                <h4 className="font-bold text-base text-slate-100">{selectedInvoice.customerName}</h4>
                <p className="text-slate-400">{selectedInvoice.customerEmail || 'billing@client.com.my'}</p>
                <p className="text-slate-400">Kuala Lumpur, Malaysia</p>
              </div>
              <div className="space-y-1 text-right">
                ...
                <div>
                  ...
                  <span className={`font-bold ${selectedInvoice.status === 'overdue' ? 'text-rose-400' : 'text-slate-200'}`}>
              ...
              <table className="w-full text-left text-xs md:text-sm">
                ...
                <tbody className="divide-y divide-slate-800/60">
                  {selectedInvoice.items?.map((item) => (
                    <tr key={item.id}>
                      <td className="py-3 font-semibold text-slate-200">{item.description}</td>
                      <td className="py-3 text-center text-slate-300">{item.quantity}</td>
                      <td className="py-3 text-right text-slate-300">RM {item.unitPrice.toFixed(2)}</td>
                      <td className="py-3 text-right font-bold text-slate-100">RM {item.total.toFixed(2)}</td>
                    </tr>
                  )) || (
                    <tr>
                      <td className="py-3 font-semibold text-slate-200">Professional Catering & Beverage Supply</td>
                      <td className="py-3 text-center text-slate-300">1</td>
                      <td className="py-3 text-right text-slate-300">RM {selectedInvoice.amount.toFixed(2)}</td>
                      <td className="py-3 text-right font-bold text-slate-100">RM {selectedInvoice.amount.toFixed(2)}</td>
                    </tr>
                  )}
                </tbody>
              </table>
            ...
            <div className="py-6 flex justify-end">
              <div className="w-64 space-y-2 text-xs md:text-sm">
                <div className="flex justify-between text-slate-400">
                  <span>Subtotal:</span>
                  <span className="text-slate-200">RM {(selectedInvoice.amount / 1.06).toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-slate-400">
                  <span>SST (6%):</span>
                  <span className="text-slate-200">RM {(selectedInvoice.amount - (selectedInvoice.amount / 1.06)).toFixed(2)}</span>
                </div>
                <div className="flex justify-between font-bold text-base text-slate-100 pt-2 border-t border-slate-800">
                  <span>Total Due:</span>
                  <span className="text-indigo-400">RM {selectedInvoice.amount.toFixed(2)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-slate-800 flex flex-wrap justify-between items-center gap-3">
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-gradient-to-r from-indigo-600 to-purple-600 hover:brightness-110 text-white text-xs font-bold rounded-xl transition-all flex items-center gap-1.5 shadow-sm"
                >
