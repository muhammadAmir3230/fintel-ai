  ...
  return (
    <div className="space-y-6 select-none">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="font-bold text-2xl md:text-3xl text-slate-100 tracking-tight">Bank & Cash Accounts</h2>
          <p className="text-xs md:text-sm text-slate-400 mt-1">Live bank feeds and daily cash drawer reconciliation.</p>
        </div>
        <div className="flex items-center gap-2.5">
          <button
            onClick={handleReconcile}
            disabled={isReconciling}
            className="flex items-center gap-2 px-4 py-2 glass-card bg-slate-900/80 border border-slate-800 hover:bg-slate-800 text-xs font-semibold text-slate-200 rounded-xl shadow-sm transition-all"
          >
          ...
          <button className="flex items-center gap-2 px-5 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 hover:brightness-110 text-white rounded-xl text-xs font-semibold shadow-lg shadow-indigo-600/20 transition-all">
      ...
      {/* Cash Overview Bento */}
      <div className="glass-card bg-gradient-to-r from-indigo-950/40 via-slate-900/60 to-purple-950/40 border border-indigo-500/20 rounded-3xl p-6 shadow-level-1 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <span className="text-xs font-bold text-indigo-400 uppercase tracking-wider">Total Liquid Cash Available</span>
          <div className="font-bold text-3xl md:text-4xl text-slate-100 mt-1">
            RM {totalCash.toLocaleString('en-US', { minimumFractionDigits: 2 })}
          </div>
          <p className="text-xs text-slate-400 mt-1">Sufficient for 4.2 months of regular operational overhead</p>
        </div>
        <div className="flex items-center gap-2 text-xs bg-slate-950/60 px-3 py-2 rounded-xl shadow-sm text-cyan-300 font-semibold border border-cyan-500/30">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {accounts.map((acc) => (
          <div key={acc.number} className="glass-card bg-slate-900/60 rounded-2xl p-5 shadow-level-1 border border-slate-800/80 flex flex-col justify-between h-44 group hover:border-indigo-500/50 transition-all">
            ...
            <div className="pt-3 border-t border-slate-800/60 flex justify-between items-end">
              <div>
                <span className="text-[11px] text-slate-400 block">Book Balance</span>
                <span className="font-bold text-xl md:text-2xl text-slate-100">
